...
class FrenetBezierWarmStartConfig:
...
class FrenetBezierWarmStartGenerator(WarmStartGenerator):
    """Warm starts for theta = [l_ctrl, l_end, v_end]."""

    def __init__(self, config: Optional[FrenetBezierWarmStartConfig] = None):
        self.config = config or FrenetBezierWarmStartConfig()

    @property
    def name(self) -> str:
        return "frenet_bezier_warm_start"

    def supports(self, context: WarmStartContext) -> bool:
        return context.trajectory_model.name == "frenet_bezier_trajectory" and context.parameter_dim == 3
    ...
    def generate(self, context: WarmStartContext) -> np.ndarray:
        ...
        for l_end in terminal_l_values:
            ...
            for l_ctrl in self._ordered_unique([float(np.clip(value, low[0], high[0])) for value in control_values]):
                for speed in speed_values:
                    rows.append(np.array([float(l_ctrl), float(l_end), float(speed)], dtype=float))
